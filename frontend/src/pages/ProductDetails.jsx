...
const PD_SPACING_DEFAULTS = { faq: { top: 32, bottom: 32 }, reviews: { top: 30, bottom: 30 }, gallery: { top: 25, bottom: 30 }, productInfo: { top: 20, bottom: 25 } };
...
function ProductDetails() {
  ...
  const measureOverlays = useCallback(() => {
    ...
    // دابا مع الـ spacing الحقيقي (25px..) هاذشي كان كيخلي صناديق الأقسام المتجاورة تبان متداخلين)
    const toRect = (el) => {
      if (!el) return null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {galleryEnabled && (
        <SectionWrapper
          type="gallery" isPreview={isPreview} spacing={sec(productSections, "gallery")?.settings?.spacing} isHighlighted={highlightedSection === "gallery"}
          style={{ minWidth: 0 }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0, minWidth: 0 }}>
          ...
          {/* In-Page Checkout */}
          {checkoutEnabled && (
          <SectionWrapper type="checkout" isPreview={isPreview} spacing={sec(productSections, "checkout")?.settings?.spacing} isHighlighted={highlightedSection === "checkout"} style={{ order: productOrder("checkout") }} registerRef={registerSectionRef} onHoverChange={setHoveredSection}>
            <div
      ...
      {/* ── Reviews ── */}
      {sec(productSections, "reviews")?.enabled !== false && sec(productSections, "reviews") && (
        <SectionWrapper type="reviews" isPreview={isPreview} spacing={sec(productSections, "reviews")?.settings?.spacing} isHighlighted={highlightedSection === "reviews"} style={{ order: productOrder("reviews") }} registerRef={registerSectionRef} onHoverChange={setHoveredSection}>
          <ReviewsSection
            settings={sec(productSections, "reviews")?.settings}
      ...
      {sec(productSections, "faq")?.enabled !== false && sec(productSections, "faq") && (
        <SectionWrapper type="faq" isPreview={isPreview} spacing={sec(productSections, "faq")?.settings?.spacing} isHighlighted={highlightedSection === "faq"} style={{ order: productOrder("faq") }} registerRef={registerSectionRef} onHoverChange={setHoveredSection}>
          <FaqSection
